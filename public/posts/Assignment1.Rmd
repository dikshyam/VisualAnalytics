...
Using train_data to build a random forest classifier with 10 trees. Use library(randomForest). 
```{r}
set.seed(42)
...
Let's increase the number of trees in 10 increments (e.g. 40, 50, …). Based OOB error rate to evaluate the random forest classifier, let's check the optimal number of trees 
...
Let's use tuneRF() function to find the best value for mtry. Here is the code: mtry <- tuneRF(train_data[-26], train_data$gen_election, ntreeTry=n,  stepFactor=1.5, improve=0.01, trace=TRUE, plot=TRUE, , na.action=na.exclude). Replace n with the number of trees you recommended in the previous r code. 
Let's see the recommended value for mtry.
...
Using library(caret) and the code to create the confusion matrix for test_data. 
Filling out the confusion matrix in below. Use “W” as the value of option positive in confusionMatrix() function. 
```{r}
set.seed(42)
...
Let's use varImpPlot() to create the plot for variable importance. 
The five important variables when we use MeanDecreaseAccuracy are:
